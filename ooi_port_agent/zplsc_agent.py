...
class FTPClientProtocol(FTPClient):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # maintains a list of downloaded files
        self._retrieved_file_queue = collections.deque('', SEARCH_SIZE)

        self._startup = True     # ensures this only happens once, despite disconnect

    def connectionMade(self):

        if self._startup:
            self._local_directory = os.path.join(self.factory.localDirectory, self.factory.refdes)
            d = defer.execute(self._process_local_files)
            d.addErrback(self._process_local_files_errback)
            d.addCallback(lambda ignore: self._change_directory())
            self._startup = False
        else:
    ...
    def _process_local_files(self):
        ...
        for root, dirnames, filenames in os.walk(self._local_directory):
            for filename in fnmatch.filter(filenames, '*.raw'):
                ...
                if original_filename not in self._retrieved_file_queue:
                    self._retrieved_file_queue.append(original_filename)
        ...
        log.msg('Number of existing data files on local disk (%s agent): ' % self.factory.refdes,
                len(self._retrieved_file_queue))
    ...
    def _retrieve_files(self, file_list_protocol):
        ...
        for file_list_line in file_list_protocol.files:
            ...
            if file_name in self._retrieved_file_queue:
                continue
        ...
        if d:
            ...
            d.addBoth(lambda ignore: self._list_files())
        else:
    ...
    def _file_retrieved(self, fileobject, filesize):
        ...
        if filesize == os.path.getsize(fileobject.name):
            ...
            self._retrieved_file_queue.append(os.path.basename(new_filename).split('_')[-1])
