...
class CamdsPortAgent(TcpPortAgent):
    ...
    def _start_smb_connection(self):
        ...
        download_factory.ref_des = self.name
        download_factory.img_dir_root = self.img_dir_root
...
class RetrieveFileFactory(SMBProtocolFactory):
    ...
    def onAuthFailed(self):
        ...
        download_factory.ref_des = self.ref_des
        download_factory.img_dir_root = self.img_dir_root

        reactor.connectTCP(self.instrument_ip, 139, download_factory)

    def create_image_dir(self):
        self.image_dir = IMG_DIR_ROOT + self.ref_des + "_IMAGES"
    ...
    def fetch_file(self):
        ...
        if self.pending_file_queue:
            ...
            file_obj = open(os.join(self.image_dir, file_name) + '.part', 'w')
